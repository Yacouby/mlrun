...
_last_update_time: datetime.datetime = None
...
def _push_terminal_run_notifications(
    db: mlrun.api.db.base.DBInterface, db_session, last_update_time
):
    """
...
def _generate_event_on_failed_runs(
    ...
    # TODO: delete this log after done with testing
    logger.info(f"number of failed runs: {len(runs)}")

    for run in runs:
        project = run["metadata"]["project"]
        entity = {"kind": "job", "project": project, "id": "*"}
        event_data = Event(kind="failed", entity=entity)
        mlrun.get_run_db().generate_event("failed", event_data)

        mlrun.api.crud.Events().process_event(
            session=db_session,
